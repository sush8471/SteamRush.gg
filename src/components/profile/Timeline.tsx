...
const Timeline: React.FC<TimelineProps> = ({ events }) => {
  const getEventColor = (type: TimelineEvent['eventType']) => {
    switch (type) {
      ...
      default:
        return 'bg-[var(--accent-primary)] shadow-[0_0_10px_var(--accent-glow)]';
    }
  };
  ...
  return (
    <section className="py-16 px-4 max-w-7xl mx-auto overflow-hidden">
      <div className="flex flex-col items-center mb-12">
        ...
        <div className="w-24 h-1 bg-gradient-to-r from-transparent via-[var(--accent-primary)] to-transparent transition-all duration-1000"></div>
      </div>

      <div className="relative">
        {/* Central Vertical Line */}
        <div className="absolute left-4 md:left-1/2 transform md:-translate-x-1/2 top-0 bottom-0 w-1 bg-gradient-to-b from-transparent via-[var(--accent-primary)]/50 to-transparent shadow-[0_0_15px_var(--accent-glow)] transition-all duration-1000"></div>
          ...
          {events.map((event, index) => (
            ...
            >
              ...
              <div className="ml-12 md:ml-0 md:w-1/2 px-4 md:px-12">
                <div 
                  className={`relative p-6 rounded-2xl bg-white/5 backdrop-blur-md border border-white/10 transition-all duration-300 group-hover:bg-white/10 group-hover:border-white/20 group-hover:shadow-[0_0_20px_var(--accent-glow)] ${
                ...
                >
                  ...
                  <h3 className="text-xl font-bold text-white mb-2 uppercase tracking-wide group-hover:text-[var(--accent-primary)] transition-colors duration-300">
                    {event.title}
                  </h3>
